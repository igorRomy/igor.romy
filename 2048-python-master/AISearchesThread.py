...
class AISearchGridThread(Thread):
    """
    ...
    def __init__(self, matrix, list, key, parent_game_grid, search_function, game_score_alg, score_type, depth):
        ...
        self.game_score_alg = game_score_alg
        self.score_type = score_type
        self.depth = depth
        # Initializing of Thread
        Thread.__init__(self)

    def run(self):
        """
        Runs the thread when .start() is invoked
        :return:
        """
        self.grid_cells = []
        ...
        getattr(self.parent_game_grid, self.score_type)(self.key, getattr(self, self.search_function)(self.key,
                                                                                    self.game_score_alg, self.depth))
    ...
    def key_down_char_with_matrix(self, key, given_matrix):
        ...
        if done:
            # Logic.[direction] worked = add new tile (game rules)
            self.matrix = Logic.add_two(given_matrix)
            # NOT USED, record last move for potential back track
            # self.history_matrix.append(self.matrix)
            # ONLY used in UI
            # self.update_grid_cells()
